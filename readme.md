...
- **Toggle Bluetooth** — runs `on.sh`, then connects to the first paired remote
  ...
  so it drops any existing connection. Wi-Fi has to be on: Bluetooth shares
  the Sage's chip with Wi-Fi, and turning Wi-Fi off powers the whole chip
  down.
...
- **Bluetooth needs Wi-Fi on.** They share the Sage's chip, and KOReader's
  Wi-Fi off powers the whole chip down. Turning Wi-Fi off, or back on, drops
  the remote without anything noticing; *Toggle Bluetooth* off and on
  afterwards. Wi-Fi only has to be on, not connected.
